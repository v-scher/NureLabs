package ua.nure.course5.MMO.lab1.Ilinskyi;

/**
 * Created by Vitalii Ilinskyi
 */
public class Algoritm{
    ...
    public static void main(String[] args) {
        ...
        String[] Versh = new String[6];

        int CH = 1; // Текущая вершина для проверки
       ...
       // }
        ...
        for (int i = CH + 1; i < 6; i++) {
            if (matr[CH][i] + Result[0][CH] < Result[0][i]) {
               //System.out.print(Result[0][i] + "i" + matr[CH][i] + " ");
                Result[0][i] = matr[CH][i] + Result[0][CH];
                Versh[i] = Versh[i] + "-" + Versh[CH];
            }
            else{
                Result[0][i] = Result[0][CH];
            }
